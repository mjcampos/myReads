...
class BooksApp extends React.Component {
  ...
  onHandleSelection = (title, type, selectedOption) => {
    var state = this.state;
    var newBookList, bookObj;

    if (selectedOption === "currentlyReading") {
        newBookList = state[type].filter(book => {
            if (book.title === title) {
                bookObj = book;
                return false;
            };
        ...
        state[type] = newBookList;
        state[selectedOption].push(bookObj);
        this.setState(state);
    } else if(selectedOption === "wantToRead") {
        newBookList = state[type].filter(book => {
            if (book.title === title) {
                bookObj = book;
                return false;
        ...
        // Remove book from its list
        newBookList = state[type].filter((book) => book.title !== title);
                    ...
                    <div className="bookshelf-books">
                        <ol className="books-grid">
                          {this.state.currentlyReading.map((book) => (
                            <Book key={book.title} handleSelect={this.onHandleSelection} type={"currentlyReading"} book={book}/>
                          ))}
                        </ol>
                    </div>
                </div>
                  ...
                  <div className="bookshelf-books">
                    <ol className="books-grid">
                        {this.state.wantToRead.map((book) => (
                            <Book key={book.title} handleSelect={this.onHandleSelection} type={"wantToRead"} book={book}/>
                        ))}
                    </ol>
                  </div>
                </div>
                  ...
                  <div className="bookshelf-books">
                    <ol className="books-grid">
                        {this.state.read.map((book) => (
                            <Book key={book.title} handleSelect={this.onHandleSelection} type={"read"} book={book}/>
                        ))}
                    </ol>
                  </div>
                </div>
